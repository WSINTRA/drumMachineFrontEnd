...
//This function for preview in dropDown-Content of add new kit
const playAudio = () => {
...
const saveNewKitToDB = () => {
  ...
  //Backend uses the title of a new drumKit to link together the sound files
  postNewKit(kitTitle)
    .then((newKit) => {
    ...
    //For now just alert new kit added
...
function componentToAddNewKit(preSavedSounds) {
  const createOptionsFromList = () => {
    ...
    preSavedSounds.forEach(
      (sound, index) =>
        //Using faker data to name the sound files that exist since they are all just url's to mp3's
        (optionString += `<option value='${index}'>${faker.random.word()}</option>`)
    );
    return optionString;
  };
  ...
  //TODO: add a button for adding sound urls, user can add their own remote hosted mp3's
  const component = `<form class="form-newkit">
                        ...
                        <div class="form-group">
                            ...
                            <div class="icon" onclick="pushNewPadToTemp()">
                                <h4 style="padding: 1rem;"><i class="fa fa-plus-square-o" aria-hidden="true"></i>
                                    Add Sound to Pad
                                </h4>
                            </div>  
                        </div>
                    </form>
                    <div class="temp-pads" id="temp-kit">
